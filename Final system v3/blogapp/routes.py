...
from blogapp.forms import LoginForm, ERForm, CRForm, PetAccountForm, CustomerAccountForm, REForm, RSForm, PetAddForm, CustomerPasswordForm, PetChangeForm,PostForm, ManageForm
	...
	if not session.get("USERNAME") is None:
		...
		if customer_db:
			if form.validate_on_submit():
				...
				db.session.commit()
				return redirect(url_for('pet_account'))
			return render_template('pet_add.html', title='pet', user=user, username=username, form=form, language=language[render_languages()])
		else:
			flash("User not permitted")
			return redirect(url_for('login'))
	else:
...
def status_a():
	...
	if not session.get("USERNAME") is None:
		...
		if customer_db:
			...
			return render_template('status_a.html', title='status', user=user, username=username, prev_posts=prev_posts, customer_in_db=customer_db, language=language[render_languages()] )
		else:
			flash("User not permitted")
			return redirect(url_for('login'))
	else:
...
def status_e():
	...
	if not session.get("USERNAME") is None:
		...
		if customer_db:		
			prev_posts = db.session.query(Pet, Appointment).filter(Pet.id == Appointment.pet_id ).all()
			return render_template('status_e.html', title='status', user=user, username=username, prev_posts=prev_posts, customer_in_db=customer_db, language=language[render_languages()] )
		else:
			flash("User not permitted")
			return redirect(url_for('login'))
	else:
...
def status_sur():
	...
	if not session.get("USERNAME") is None:
		...
		if customer_db:		
			prev_posts = db.session.query(Pet, Appointment).filter(Pet.id == Appointment.pet_id ).all()
			return render_template('status_sur.html', title='status', user=user, username=username, prev_posts=prev_posts, customer_in_db=customer_db, language=language[render_languages()] )
		else:
			flash("User not permitted")
			return redirect(url_for('login'))
	else:
	...
	if not session.get("USERNAME") is None:
		...
		if employee_in_db:
			if form.validate_on_submit():
				...
				user = {'city': employee_in_db.key}
				prev_posts = db.session.query(Customer, Appointment).filter(Appointment.customer_id == Customer.id).filter(Appointment.city==employee_in_db.key).order_by(Appointment.type).all()
...
def employee_f():
	...
	if not session.get("USERNAME") is None:
		...
		if employee_in_db:
			...
			prev_posts = db.session.query(Customer, Appointment).filter(Appointment.customer_id == Customer.id).filter(Appointment.city==employee_in_db.key).order_by(Appointment.id.desc()).paginate(page, 6)
